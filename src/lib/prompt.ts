...
- Добавь поле "dict": объект «слово → перевод» для ВСЕХ остальных слов текста (ключи в нижнем регистре, без имён собственных). Переводи по роли слова в этом тексте: «die Bank» в парке — «скамейка». Части составных форм объясняй с отсылкой к целому: для «hat … gekauft» → "hat": "вспомогательный глагол (hat gekauft = купила)", "gekauft": "купила (Perfekt от kaufen)". ВАЖНО про ОТДЕЛЯЕМЫЕ глаголы: если приставка стоит позже в предложении («gibt … auf», «zieht … an», «steht … auf», «ruft … an»), переводи глагол по смыслу ОТДЕЛЯЕМОГО глагола и называй инфинитив: "gibt": "сдаётся (aufgeben: gibt … auf)", "zieht": "надевает (anziehen: zieht … an)". НЕ давай базовое значение («даёт», «тянет», «стоит»), если по контексту это отделяемый глагол; и обязательно помечай приставку маркером {{+auf}}.
- Предлог при глаголе переводи В КОНТЕКСТЕ и глоссируй ИНЛАЙН в тексте, а не общим значением в dict. Если глагол управляет предлогом с неочевидным смыслом (denken an — думать о, warten auf — ждать, sich freuen über/auf — радоваться, fragen nach — спрашивать о, sich interessieren für — интересоваться, teilnehmen an — участвовать в), помечай это прямо в предложении: «er {{denkt|думает о (denken an)}} {{+an}} das Wasser». Один и тот же предлог часто значит РАЗНОЕ в разных местах текста (an der Tür — у двери; denkt an — думает о; zieht … an — надевает), поэтому такие случаи всегда решай инлайном.
- В dict у каждого слова — ОДНО чистое значение в его роли в тексте. Не сваливай в одну статью несколько смыслов и не добавляй отсылок к другим словам («также: …», «здесь: … другой глагол»). Для служебных слов (an, auf, zu, in) оставляй в dict базовое значение предлога — особые смыслы этого же слова уже помечены инлайн там, где встречаются.
...
const LEVEL_RUBRIC = `Уровень ("level") определяй по ГРАММАТИКЕ и лексике, не по длине текста:
- A1: только Präsens (чуть Perfekt); простые главные предложения, порядок SVO; частотная бытовая лексика; вопросы с W-словами; sein/haben, модальные können/wollen; почти без придаточных.
- A2: уверенный Perfekt и Präteritum частотных глаголов; модальные во всех формах; дательный падеж; отделяемые глаголы; придаточные weil/dass/wenn; сравнительная степень; возвратные глаголы.
- B1: свободное повествование в Präteritum; Konjunktiv II (würde/hätte/könnte); пассив; относительные придаточные; Genitiv; obwohl/damit/während; более абстрактные темы.
- B2+: сложный синтаксис, Konjunktiv I, номинализация, идиоматика, абстрактные/специальные темы.
Ставь уровень по самому сложному, что РЕГУЛЯРНО встречается (единичный оборот выше планку не поднимает). Можно диапазон: "A1", "A1-A2", "A2", "B1".`
...
- Уровень A1–A2: простая лексика, простые предложения, в основном Präsens (немного Perfekt — можно). В поле "level" поставь ФАКТИЧЕСКИЙ уровень получившегося текста по рубрике:
${LEVEL_RUBRIC}
...
- Определи уровень текста и запиши его в поле "level" по рубрике:
${LEVEL_RUBRIC}
